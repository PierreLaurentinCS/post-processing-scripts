...
def bubble_volume(output_path):
    ...
    # if not os.path.isfile(output_path + '/phase_statistics.dat'):
    #     print("test")
    #     return [],[],[],[]
    ...
    time_integrated_flux_volume = computed_volume(list_of_list_of_vars_flux[0][0],list_of_list_of_vars_flux[0][3],list_of_list_of_vars[0][6][0])
    analytical_volume_array = analytical_volume(list_of_list_of_vars[0][0],5.0e2,(2*np.pi*(0.5)**3)/3)
...
def deriv(f,t):
    if len(f)!=len(t):
        raise Exception("Time list length and bubble volume list length are not equal!")

    deriv_list = (f[:-1]-f[1:])/(t[:-1]-t[1:])
    return deriv_list
...
def get_numerical_detachment_time(output_path):
    ...
    if os.path.isfile(output_path + '/contour_area_array.npy') and os.path.isfile(output_path + '/time.npy'):
        print('Reading the contour area file from an external source')
        contour_area_array = np.load(output_path + '/contour_area_array.npy')
        ...
        contour_area_array = np.zeros(len(time))
        with alive_bar(len(time),refresh_secs=0) as bar:
            for i, vtu_file in enumerate(list_vtu):
                ...
                contour_area_array[i] = contour_area
                bar()

        np.save(output_path + '/time.npy', time)
        np.save(output_path + '/contour_area_array.npy', contour_area_array)

    contour_area_derivative = deriv(contour_area_array, time)
    time_cut = 1
    x,y = get_contour_at_detachment(np.argmin(np.abs(contour_area_derivative[np.where(time < time[-time_cut])])),list_vtu,output_path)
    detachment_time = time[np.argmin(np.abs(contour_area_derivative[np.where(time < time[-time_cut])]))]

    return time[np.where(time < time[-time_cut])], contour_area_array[np.where(time < time[-time_cut])], contour_area_derivative[np.where(time < time[-time_cut])], detachment_time,x,y
